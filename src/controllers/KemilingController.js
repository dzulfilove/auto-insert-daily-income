...
const { connectionKemiling } = require("../config/Database.js");
const {
  getDataBulan,
  getDataHarian,
  handleAddBulanan,
...
const dataKlinik = [
  {
    nama: "Klinik Pratama Kosasih Kemiling",
    id: 1,
    akun: "",
    jnsAkun: "Klinik",
  },
  {
    nama: "Laboratorium Kosasih Kemiling",
    id: 2,
    akun: "",
    jnsAkun: "Lab",
  },
  ...
  try {
    ...
    const promises = dataKlinik.map(async (klinik) => {
      ...
      const response = await fetch.default(
        `http://202.157.189.177:5005/kemiling/pendapatan/${klinik.akun}`
      );
