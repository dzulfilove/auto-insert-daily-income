...
const dataKlinik = [
  {
    nama: "Griya Terapi Sehat Tirtayasa",
    id: 17,
    akun: "",
    jnsAkun: "Klinik",
  },
...
const getPendapatan = async (req, res) => {
  ...
  try {
    const [resultPendapatanBarangKlinik] = await connectionGtsTirtayasa.query(
      queryGetPendapatanBarangKlinik
    );
    const [resultPendapatanJasaKlinik] = await connectionGtsTirtayasa.query(
  ...
  try {
    ...
    const promises = dataKlinik.map(async (klinik) => {
      ...
      const response = await fetch(
        `http://localhost:5005/gts-tirtayasa/pendapatan/${klinik.akun}`
      );
