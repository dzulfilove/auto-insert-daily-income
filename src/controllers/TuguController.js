...
const { connectionTugu } = require("../config/Database.js");
const {
  getDataBulan,
  getDataHarian,
  handleAddBulanan,
...
const namaGigi = "Klinik Pratama Kosasih Tugu (gigi)";
const idCabang = [12, 13, 14];
const dataKlinik = [
  {
    nama: "Klinik Pratama Kosasih Tugu",
    id: 12,
    akun: "",
    jnsAkun: "Klinik",
  },
  {
    nama: "Laboratorium Kosasih Tugu",
    id: 13,
    akun: "",
    jnsAkun: "Lab",
  },
  {
    nama: "Klinik Pratama Kosasih Tugu (gigi)",
    id: 14,
    akun: "",
    jnsAkun: "Gigi",
  },
...
const getPendapatan = async (req, res) => {
  ...
  try {
    ...
    const [resultPendapatanBarangGigi] = await connectionTugu.query(
      queryGetPendapatanBarangGigi
    );
    const [resultPendapatanJasaGigi] = await connectionTugu.query(
  ...
  try {
    ...
    const promises = dataKlinik.map(async (klinik) => {
      ...
      const response = await fetch(
        `http://202.157.189.177:5005/tugu/pendapatan/${klinik.akun}`
      );
