...
const dataKlinik = [
  {
    nama: "Klinik Pratama Kosasih Teluk",
    id: 19,
    akun: "",
    jnsAkun: "Klinik",
  },
...
const getPendapatan = async (req, res) => {
  ...
  try {
    const [resultPendapatanBarangKlinik] = await connectionTeluk.query(
      queryGetPendapatanBarangKlinik
    );
    const [resultPendapatanJasaKlinik] = await connectionTeluk.query(
  ...
  try {
    ...
    const promises = dataKlinik.map(async (klinik) => {
      ...
      const response = await fetch(
        `http://localhost:5005/teluk/pendapatan/${klinik.akun}`
      );
